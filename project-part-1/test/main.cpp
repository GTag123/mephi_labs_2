...
size_t parseInt(const string &contents, size_t index, size_t &value) {
    size_t i = index;
    value = 0;
...
size_t getNextNode(const string &contents, size_t index) {
    if (contents[index] == 'd') {
        index++;
...
int main() {
    std::ifstream file("./ah.torrent", std::ios::binary);
    ...
    size_t piece_length;
    size_t length;
    auto i = parse_info_dict(contents, infohash, announce, comment, piece_hashes, piece_length, length);
    // print all
    ...
    std::cout << "infohash: ";
    for (int i = 0; i < 20; i++) {
        std::cout << std::hex << (int) infohash[i];
    }
    std::cout << std::endl;
}