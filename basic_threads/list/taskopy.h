...
//        std::unique_lock<std::shared_mutex> taillock(tailListMutex);
    ...
    public:
        ...
        ~Iterator(){
            destcount++;
//            std::cout << "Destructed " + std::to_string(destcount) << std::endl;
//            if (curr_ != nullptr) {
            if (isRefLocked){
                curr_->nodemutex_.unlock();
            }
//            }
